...
func TestHTMLAutoExpandsOnlyInLargeViewport(t *testing.T) {
	html := HTML("recall", "Recall")
	for _, marker := range []string{
		`AUTO_EXPAND_MIN_HEIGHT=500`,
		`AUTO_EXPAND_MIN_WIDTH=640`,
		`window.innerHeight>=AUTO_EXPAND_MIN_HEIGHT&&window.innerWidth>=AUTO_EXPAND_MIN_WIDTH`,
		`window.addEventListener("resize",tryAutoExpand`,
		`if(userSetShellExpanded||shellExpanded||!currentShell||!isLargeViewport())return;`,
		`shellExpanded=true;`,
	} {
		if !strings.Contains(html, marker) {
			t.Fatalf("shared MCP App missing auto-expand marker %q", marker)
		}
	}
}

func TestHTMLPreservesShellStateAcrossResizeAndRender(t *testing.T) {
	html := HTML("recall", "Recall")
	for _, marker := range []string{
		`let shellExpanded=false;`,
		`let userSetShellExpanded=false;`,
		`userSetShellExpanded=true;`,
		`shellExpanded=!shellExpanded;`,
		`currentShell={toggle,panel};`,
		`setShellExpanded(currentShell,shellExpanded);`,
		`tryAutoExpand();`,
	} {
		if !strings.Contains(html, marker) {
			t.Fatalf("shared MCP App missing shell-state marker %q", marker)
		}
	}
}
